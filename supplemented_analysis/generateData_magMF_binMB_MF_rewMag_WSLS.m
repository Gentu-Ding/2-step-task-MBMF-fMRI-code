...
%    wMagState       = fitData.transParams(4);
    ...
    %fitData.transParams(5) =  exp(params(5));
    %wMag = fitData.transParams(5);
    wMag=0;
    ...
    fitData.qDiff_ChosenUnchosen       = nan( size(dataTID,1), 1 );   
    
    fitData.wWSLS       =  nan( size(dataTID,1), 1 );   
    fitData.ChosenV    =  nan( size(dataTID,1), 1 );   
    fitData.UnchosenV    = nan( size(dataTID,1), 1 );   
    ...
    %        prevMagState(:)     = 0;
     ...
     %       magStimDiff     = wMagStim * (prevMagStim(1) - prevMagStim(2));
            ...
            if choice_1 ==1
                ...
                fitData.qDiff_ChosenUnchosen(tI,1) =  qDiff;
                
                fitData.wWSLS(tI,1) = MF_WSLS;
                

                fitData.ChosenQ(tI,1) = mfQ1(1);
                fitData.UnchosenQ(tI,1) = mfQ1(2);
                
                
                
                fitData.ChosenV(tI,1) = mfQ1(1) + wWSLS_MF(1);
                fitData.UnchosenV(tI,1) = mfQ1(2) + wWSLS_MF(2);
                
            else
                fitData.qDiff_ChosenUnchosen(tI,1) =  -qDiff;
                fitData.wWSLS(tI,1) = -MF_WSLS;
            
                fitData.ChosenQ(tI,1) = mfQ1(2);
                fitData.UnchosenQ(tI,1) = mfQ1(1);
                
                fitData.ChosenV(tI,1) = mfQ1(2) + wWSLS_MF(2);
                fitData.UnchosenV(tI,1) = mfQ1(1) + wWSLS_MF(1);
            ...
            fitData.value_update(tI,1) = rl_MF * fitData.mfRPE4(tI,1);
...
%             pTrans_1(altStateIndex) = pTrans_1(altStateIndex) * (1-aSPE1);
            ...
            if choice_1 ==1
                ...
                fitData.Chosen_wsls(tI,1) =  wWSLS_MF(1);
                
                fitData.Unchosen_wsls(tI,1) =  wWSLS_MF(2);
                
                
                
                fitData.pChosenQ(tI,1) = mfQ1(1);
                fitData.pUnchosenQ(tI,1) = mfQ1(2);
                
     
                fitData.pChosenV(tI,1) = mfQ1(1) + wWSLS_MF(1);
                fitData.pUnchosenV(tI,1) = mfQ1(2) + wWSLS_MF(2);
                               
            else
            
                fitData.Chosen_wsls(tI,1) =  wWSLS_MF(2);
                
                fitData.Unchosen_wsls(tI,1) =  wWSLS_MF(1);
                
                fitData.pChosenQ(tI,1) = mfQ1(2);
                fitData.pUnchosenQ(tI,1) = mfQ1(1);
                
                fitData.pChosenV(tI,1) = mfQ1(2) + wWSLS_MF(2);
                fitData.pUnchosenV(tI,1) = mfQ1(1) + wWSLS_MF(1);
