function trials = buildTrials_dynamic(taskStruct, numTrials)
    ...
    minDuration_contingent = round(numTrials/2);
    maxDuration_contingent = minDuration_contingent + 1;
    ...
    % for reward contingency (stimulus or state) condition
    contingentOrder = randsample([taskStruct.CONT_STIM, taskStruct.CONT_STATE], 2);
    contingentCount = 0;
    contingentDuration = randi([minDuration_contingent, maxDuration_contingent]);
    trials.condContingent = nan(numTrials, 1);
    ...
    for tI = 1 : numTrials
        ...
        contingentCount = contingentCount + 1;
        ...
        % check to see if we should flip the reward contingency
        if contingentCount > contingentDuration
            % implement a condition switch
            contingentOrder = fliplr(contingentOrder);
            contingentDuration = randi([minDuration_contingent, maxDuration_contingent]);
            contingentCount = 0;
        end
        ...
        trials.condContingent(tI) = contingentOrder(1);
    end % for each trial
