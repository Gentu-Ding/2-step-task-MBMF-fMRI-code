function [task_MBMF_fMRI, ioStruct] = run_SpaceMarket_fMRI(subID, orderID, wPtr)
    ...
    for rI = unique(task_MBMF_fMRI.trials.runID)'
        ...
        % wait for scanner pulse to begin task
        task_MBMF_fMRI.tRunStart(rI) = waitForScannerPulse(ioStruct);
        % run the block of trials
        task_MBMF_fMRI = startSpaceMarketRun(task_MBMF_fMRI, ioStruct, runTrials, Inf, true);
