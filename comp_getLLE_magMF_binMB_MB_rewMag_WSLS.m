...
%    wMagState       = fitData.transParams(4);
    ...
    fitData.transParams(3) =  params(3);
    wWS_MB          = fitData.transParams(3);
    
    fitData.transParams(4) =  params(4);
    wLS_MB          = fitData.transParams(4);
    % anchore MF weight
...
%     
    ...
    fitData.chosen_mfUtility        = nan(size(dataTID,1), 1);
    fitData.chosen_mbUtility        = nan(size(dataTID,1), 1);
        
    fitData.unchosen_mfUtility        = nan(size(dataTID,1), 1);
    fitData.unchosen_mbUtility        = nan(size(dataTID,1), 1);
...
%             prevMagState(:)     = 0.5;
        ...
        if ~isnan(data.resp1(tI)) % && data.TOI(tI)~=0
            ...
            fitData.chosen_mfUtility(tI,1) = fitData.mfUtility(tI,data.resp1(tI));
            fitData.chosen_mbUtility(tI,1) = fitData.mbUtility(tI,data.resp1(tI));
                      
            fitData.unchosen_mfUtility(tI,1) = fitData.mfUtility(tI,3-data.resp1(tI));
            fitData.unchosen_mbUtility(tI,1) = fitData.mbUtility(tI,3-data.resp1(tI));
